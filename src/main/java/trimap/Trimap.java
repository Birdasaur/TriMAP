...
//    int sgn(T val);
        ...
        Vector<Vector<Integer>> triplets;
        Vector<Double> weights;
        Vector<Vector<Double>> embedding;

        Vector<Vector<Integer>> neighbour_index;   // n_samples x m_large
...
//
//        void gd(vector<vector<double>> &tmp,
...
//    };
    ...
    public Trimap(int n_dims, int m, int mi, int r, int out_dims) 
//    : n_dims(n_dims), m(m), mi(mi), r(r), out_dims(out_dims)
    {};

    public Vector<Vector<Double>> run(Vector<Vector<Double>> input_data) {
...
void Trimap::calculate_weights() {
        ...
        vector<double> sigmas(n_samples
        ); // average of distance from 4th to 6th neighbour
  weights.resize(n_triplets);
    ...
    return log(1 + gamma_factor * (x / scaling_factor + delta_constant));
        ...
        double scaling_factor =  * std::max_element
        (weights.begin()
        , weights.end()
        );
  ...
  for (int i = 0; i < n_triplets; i++) {
        ...
        double max_weight =  * std::max_element
        (weights.begin()
        , weights.end()
        );
  for (int i = 0; i < n_triplets; i++) {
    ...
    void Trimap::make_embedding() {
        ...
        vector<vector<double>> velocity(n_samples, vector<double>(out_dims
        , 0.0));
  vector<vector<double>> gain(n_samples, vector<double>(out_dims
        , 1.0));

  double gamma = 0.3;

        for (int iter = 1; iter <= iterations; iter++) {
            vector<vector<double>> tmp(n_samples, vector<double>(out_dims
            , 0.0));
        ...
        )
{
  ...
  int n_samples = input.size();
        ...
        vector<vector<double>> grad(n_samples, vector<double>(out_dims
        , 0.0));
...
double Trimap
    ...
    ::euclid_dist(vector<double> a, vector<double> b) {
